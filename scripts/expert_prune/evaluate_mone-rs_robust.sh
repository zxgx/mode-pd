...
for subdir in zyda2-500 zyda2-1000 c4-100 c4-500 c4-1000; do
    ...
    torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
        --hf_model exp/$subdir/mone_rs/$model_id-mone-pruned-48 $eval_config \
        --batch_size 8 --trust_remote_code --output_dir exp/$subdir/mone_rs

    torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
        --hf_model exp/$subdir/mone_rs/$model_id-mone-pruned-32 $eval_config \
        --batch_size 8 --trust_remote_code --output_dir exp/$subdir/mone_rs

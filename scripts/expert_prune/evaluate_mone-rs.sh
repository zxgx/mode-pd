...
torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
    --hf_model exp/mone_rs/$model_id-mone-pruned-48 $eval_config \
    --batch_size 8 --trust_remote_code --output_dir exp/mone_rs

torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
    --hf_model exp/mone_rs/$model_id-mone-pruned-32 $eval_config \
    --batch_size 8 --trust_remote_code --output_dir exp/mone_rs

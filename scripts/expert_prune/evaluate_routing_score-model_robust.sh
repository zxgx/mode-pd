...
for subdir in c4-1000; do
    # model_id="OLMoE-1B-7B-0125"
    # torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
    #     --hf_model exp/$subdir/routing_score_model/$model_id-pruned-48 $eval_config \
    #     --batch_size 8 --trust_remote_code --output_dir exp/$subdir/routing_score_model

    # torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
    #     --hf_model exp/$subdir/routing_score_model/$model_id-pruned-32 $eval_config \
    #     --batch_size 8 --trust_remote_code --output_dir exp/$subdir/routing_score_model

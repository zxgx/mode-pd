...
for subdir in zyda2-500 zyda2-1000 c4-100 c4-500 c4-1000; do
    ...
    torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
        --hf_model exp/$subdir/sparsegpt/$model_id-sparsegpt-pruned-75 $eval_config \
        --batch_size 8 --trust_remote_code --output_dir exp/$subdir/sparsegpt

    torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
        --hf_model exp/$subdir/sparsegpt/$model_id-sparsegpt-pruned-50 $eval_config \
        --batch_size 8 --trust_remote_code --output_dir exp/$subdir/sparsegpt

...
for metric in normalized_scores logsum_scores weighted_sum_p25 weighted_sum_p50 weighted_sum_p75; do
...
torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
    --hf_model iclr_rebuttal/metric_ablation/$metric/$model_id-mone-pruned-48 \
    --tasks $EVAL_TASKS --num_fewshots $EVAL_FEWSHOTS \
    --batch_size 8 --trust_remote_code --output_dir iclr_rebuttal/metric_ablation/$metric
EOF

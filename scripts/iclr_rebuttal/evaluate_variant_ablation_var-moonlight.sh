...
torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
    --hf_model iclr_rebuttal/variant_ablation_var/$dataset-$sample_size/$model_id-mone-pruned-48 \
    --tasks $EVAL_TASKS --num_fewshots $EVAL_FEWSHOTS \
    --batch_size 8 --trust_remote_code --output_dir iclr_rebuttal/variant_ablation_var/$dataset-$sample_size
EOF

singularity exec --nv $image bash << EOF
source $HPCTMP/venvs/mone/bin/activate
torchrun --standalone --nproc_per_node $GPUS_PER_NODE modepd/eval.py \
    --hf_model iclr_rebuttal/variant_ablation_var/$dataset-$sample_size/$model_id-mone-pruned-32 \
    --tasks $EVAL_TASKS --num_fewshots $EVAL_FEWSHOTS \
    --batch_size 8 --trust_remote_code --output_dir iclr_rebuttal/variant_ablation_var/$dataset-$sample_size
EOF

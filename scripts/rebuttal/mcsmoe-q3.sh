...
python modepd/prune.py --model_name_or_path QWen/$model_id $data_config \
    --expert_prune --expert_prune_metric mc_smoe --preserve_n_experts 96 \
    --compressed_model_save_path exp/mc_smoe/$model_id-mc_smoe-pruned-96
EOF

singularity exec --nv $image bash << EOF
source $HPCTMP/venvs/mone/bin/activate
python modepd/prune.py --model_name_or_path QWen/$model_id $data_config \
    --expert_prune --expert_prune_metric mc_smoe --preserve_n_experts 64 \
    --compressed_model_save_path exp/mc_smoe/$model_id-mc_smoe-pruned-64
EOF

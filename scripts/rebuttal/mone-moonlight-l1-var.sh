...
#PBS -j oe
#PBS -k oed
#PBS -o mone-moonlight-l1-var.log 
...
python modepd/prune.py --model_name_or_path moonshotai/$model_id $data_config \
    ...
    --compressed_model_save_path exp/ablation/$model_id-mone-pruned-48-l1-var \
    --l1_norm

python modepd/prune.py --model_name_or_path moonshotai/$model_id $data_config \
    --expert_prune --preserve_n_experts 32 --expert_ranking_scope layer \
    --expert_prune_metric mone --mone_ranking_metric fusion \
    --compressed_model_save_path exp/ablation/$model_id-mone-pruned-32-l1-var \
    --l1_norm
EOF

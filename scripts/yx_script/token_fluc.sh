...
for preserve_n in 48 32; do
    ...
    echo "Running expert pruning (token model) with preserve_n: $preserve_n"
    ...
    mpirun --hostfile $HOSTFILE --np $NNODES -N 1 \
        ...
        --mone_ranking_metric token_fluctuation\
        --compressed_model_save_path $SCRATCH/DeepSeek-V2-Lite-Chat-Compressed/expert_prune_token_model_${preserve_n} &&\
        python modepd/eval.py\
        --hf_model $SCRATCH/DeepSeek-V2-Lite-Chat-Compressed/expert_prune_token_model_${preserve_n}\
        --tasks ai2_arc boolq copa mmlu openbookqa piqa rte winogrande \
        --num_fewshots 0 0 0 0 0 0 0 0 \
        --trust_remote_code \
        --output_dir $SCRATCH/logs > expert_prune_token_model_${preserve_n}.log 2>&1"
